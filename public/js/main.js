$(function() {
	...
	var handlers = {
		...
		'new.message': function(data) {
			var msg = $('<div>').text((data.client.name || data.client.id) + ': ' + data.text);
			msg.insertBefore(msgInput);
			appContainer.scrollTop(appContainer[0].scrollHeight);
		}
	};

	$(document).on('resize', function() {
		setTimeout(function() {
			appContainer.scrollTop(appContainer[0].scrollHeight - appContainer.height());
		},0);
	});
	...
	var appContainer = $('<div>')
		...
		.html('Your app will be here.')
		.appendTo('body');

	var msgInput = $('<input>')
		.on('keypress', function(e) {
			if (e.keyCode == 13) {
				client.send('new.message', { text: $(this).val() });
				$(this).val('');
			}
		})
